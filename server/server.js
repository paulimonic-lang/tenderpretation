...
// Real Live API Ingestion & Intelligent Valuation Engine for etenders.gov.za
app.get('/api/tenders/live', async (req, res) => {
  console.log('🇿🇦 Ingesting FULL live catalog & computing valuations from https://www.etenders.gov.za/ ...');
  
  try {
    const pageLength = req.query.limit || 2000;
    const targetUrl = `https://www.etenders.gov.za/Home/PaginatedTenderOpportunities?status=1&draw=1&start=0&length=${pageLength}`;
    ...
    console.log(`✓ SUCCESS! Loaded ${rawTenders.length} REAL LIVE tenders from etenders.gov.za (Total Active: ${response.data.recordsTotal})`);
    ...
    console.error('Error fetching live etenders.gov.za data:', error.message);
    res.status(500).json({
      success: false,
      error: error.message,
      message: 'Failed to query live etenders.gov.za endpoint.'
    });
  }
});
